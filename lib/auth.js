...
// Menus que o usuário pode acessar.
// - adm: sempre tudo (não é restringível, para não travar a administração)
// - demais: o que estiver marcado na coluna "menus"; vazio = padrão do perfil
export function menusPermitidos(usuario) {
  const p = perfilDe(usuario)
  if (p === 'adm') return MENUS.map(m => m.key)

  const permitidosPorPerfil = PADRAO_POR_PERFIL[p] || []
  const marcados = String(usuario?.menus || '')
    .split(',').map(s => s.trim()).filter(Boolean)

  if (!marcados.length) return permitidosPorPerfil

  // Dashboard e perfil são sempre acessíveis
  const lista = new Set(['dashboard', ...marcados.filter(k => permitidosPorPerfil.includes(k))])
  return [...lista]
}

export function podeAcessarMenu(usuario, chave) {
  return menusPermitidos(usuario).includes(chave)
}

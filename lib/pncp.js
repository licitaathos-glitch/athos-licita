...
async function buscarSeguindo(url) {
  ...
  for (let t = 0; t < 2; t++) {
    try {
      r = await fetchComLimite(url, { headers: HEADERS, redirect: 'follow', cache: 'no-store' }, 9000)
    } catch (e) {
  ...
  return { resposta: r || null, urlFinal: url, erro: ultimoErro }
}
...
export async function extrairPorLink(link) {
  ...
  try {
    ...
    for (const candidato of caminhos) {
      ...
      const { resposta, erro: erroConexao } = await buscarSeguindo(candidato)
      if (resposta && resposta.ok) { r = resposta; urlCompra = candidato; break }
      tentados.push(`${candidato.replace('https://pncp.gov.br', '')} → HTTP ${resposta ? resposta.status : 'sem resposta (' + (erroConexao || 'falha de conexão') + ')'}`)
    }

  ...
  for (let t = 0; t < 2; t++) {
    ...
    try {
      ...
      while (r && [301, 302, 303, 307, 308].includes(r.status) && saltos < 3) {
        ...
        // 301 sem Location é beco sem saída: registra para aparecer no
        // diagnóstico em vez de sumir como um 301 qualquer
        if (!destino) { ultimoErro = 'redirecionou sem informar o destino (301 sem Location)'; break }
...
export async function extrairPorLink(link) {
  ...
  try {
    ...
    for (const candidato of caminhos) {
      ...
      const destino = urlFinal !== candidato
        ? ` (redirecionou para ${urlFinal.replace('https://pncp.gov.br', '')})`

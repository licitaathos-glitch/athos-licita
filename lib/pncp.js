...
export async function extrairPorLink(link) {
  ...
  try {
    ...
    // da compra (pncp-api/v1, em vez de api/pncp/v1) — tentamos os dois.
    let arquivos = []
    let diagArquivos = ''
    const basesArquivos = [
    ...
    for (const base of basesArquivos) {
      try {
        const { resposta: ra } = await buscarSeguindo(base + '/arquivos?pagina=1&tamanhoPagina=50')
        if (!ra) { tentadosArquivos.push(`${base.replace('https://pncp.gov.br', '')} → sem resposta`); continue }
        if (!ra.ok) { tentadosArquivos.push(`${base.replace('https://pncp.gov.br', '')} → HTTP ${ra.status}`); continue }
        const arr = await ra.json()
        const lista = Array.isArray(arr) ? arr : (arr.data || [])
        if (!lista.length) { tentadosArquivos.push(`${base.replace('https://pncp.gov.br', '')} → vazio`); continue }
        ...
        tentadosArquivos.push(`${base.replace('https://pncp.gov.br', '')} → ${e.message}`)
      }
    }

...
export async function extrairPorLink(link) {
  ...
  try {
    ...
    // O PNCP às vezes devolve uma URL pronta, às vezes só o número sequencial
    // do documento — nesse caso, montamos o link de download nós mesmos.
    let arquivos = []
    let diagArquivos = ''
    try {
      const { resposta: ra } = await buscarSeguindo(urlCompra + '/arquivos?pagina=1&tamanhoPagina=50')
      if (!ra) {
        diagArquivos = 'Sem resposta do PNCP para a lista de documentos.'
      } else if (!ra.ok) {
        diagArquivos = `PNCP respondeu HTTP ${ra.status} ao listar documentos.`
      } else {
        const arr = await ra.json()
        const lista = Array.isArray(arr) ? arr : (arr.data || [])
        arquivos = lista.map(a => {
          const seq = a.sequencialDocumento ?? a.sequencial ?? ''
          return {
            ...
            url: a.url || a.uri || (seq !== '' ? `${urlCompra}/arquivos/${seq}` : ''),
            sequencial: seq,
          }
        }).filter(a => a.url)
        if (!arquivos.length) diagArquivos = lista.length ? 'PNCP listou documentos, mas sem como montar o link.' : 'PNCP não tem documentos publicados para esta licitação.'
      }
    } catch (e) { diagArquivos = 'Erro ao buscar documentos: ' + e.message }

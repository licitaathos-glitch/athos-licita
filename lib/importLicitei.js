...
export function filtrarELinhas(linhasPlanilha, { mes, ano, palavraEmpresa }) {
  ...
  return linhasPlanilha
    .filter(l => {
      ...
      // Processos ainda em andamento (não homologados) não têm uma "data do
      // mês" que faça sentido filtrar — a data de abertura pode até ser no
      // futuro. Esses entram sempre, independente do mês/ano escolhidos.
      const faseLicitei = String(l['Fase'] || '').trim()
      const aindaEmAndamento = faseLicitei && faseLicitei !== 'Homologada'
      if (aindaEmAndamento) return true
    ...
    .map(l => {
      ...
      registro._foraDoMes = faseLicitei && faseLicitei !== 'Homologada'
      return registro
    })
}
